...
void parallel_transaction(GraphDB& db, const FUNC_T& func, int txn_num) {
  ...
  std::shuffle(txn_ids.begin(), txn_ids.end(), std::mt19937(std::random_device()()));
...
typename TXN_T::vertex_iterator get_random_vertex(TXN_T& txn, gs::label_t label_id) {
...
void AtomicityCTest(const std::string& work_dir, int thread_num) {
  ...
  parallel_transaction(db, [&](GraphDBSession& session, int txn_id) {
    bool successful =
...
void G0(GraphDBSession& db, int64_t person1_id, int64_t person2_id, int64_t txn_id) {
  auto txn = db.GetUpdateTransaction();
  auto person_label_id = db.schema().get_vertex_label_id("PERSON");
  auto knows_label_id = db.schema().get_edge_label_id("KNOWS");
...
void G0Test(const std::string& work_dir, int thread_num) {
  ...
  parallel_transaction(db, [&](GraphDBSession& db, int txn_id) {
    std::random_device rand_dev;
    std::mt19937 gen(rand_dev());
  ...
  for (int j = 1; j <= 100; j++) {
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      CHECK(txn.AddVertex(person_label_id, vid,
                          {Any::From(id_property), Any::From(std::to_string(j)), Any::From(value)}));
      vids.push_back(vid);
    }
    for (int i = 0; i < 4; i++) {
...
void OTVTest(const std::string& work_dir, int thread_num) {
  ...
  parallel_client(db, [&](GraphDBSession& session, int client_id) {
	  ...
	  for (int i = 0; i < 1000; ++i) {
            ...
            int64_t v1_max = std::max(std::max(std::get<0>(tup1), std::get<1>(tup1)),
                                      std::max(std::get<2>(tup1), std::get<3>(tup1)));
            int64_t v2_min = std::min(std::min(std::get<0>(tup2), std::get<1>(tup2)),
...
void FRInit(GraphDB& db, const std::string& work_dir, int thread_num) { OTVInit(db, work_dir, thread_num); }
...
std::vector<std::tuple<int64_t, int64_t, int64_t, int64_t>> WS2(GraphDBSession& db) {
