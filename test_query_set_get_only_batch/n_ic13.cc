      ...
      while (true)
      {
        if (q1.size() <= q2.size())
        {
          if (q1.empty())
            break;
          person_vids.clear();
          person_vids.reserve(q1.size());
          while (!q1.empty())
          {
            person_vids.push_back(q1.front());
            q1.pop();
          }
          auto person_knows_person_out_items = txn.BatchGetOutgoingEdges<Date>(
              person_label_id_, person_label_id_, knows_label_id_, person_vids);
          auto person_knows_person_in_items = txn.BatchGetIncomingEdges<Date>(
              person_label_id_, person_label_id_, knows_label_id_, person_vids);

          for (auto person_idx = 0; person_idx < person_vids.size(); person_idx++)
          {
            auto x = person_vids[person_idx];
            auto oe = person_knows_person_out_items[person_idx];
            ...
            auto ie = person_knows_person_in_items[person_idx];
        ...
        else
        {
          if (q2.empty())
            break;
          person_vids.clear();
          person_vids.reserve(q2.size());
          while (!q2.empty())
          {
            person_vids.push_back(q2.front());
            q2.pop();
          }
          auto person_knows_person_out_items = txn.BatchGetOutgoingEdges<Date>(
              person_label_id_, person_label_id_, knows_label_id_, person_vids);
