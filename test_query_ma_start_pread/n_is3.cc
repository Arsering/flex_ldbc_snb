...
namespace gs
{
  ...
  public:
    ...
    bool Query(Decoder &input, Encoder &output) override
    {
      ...
      std::vector<gs::MutableNbr<Date>> oe;
      txn.GetOutgoingGraphView<Date>(person_label_id_, person_label_id_, knows_label_id_).read(root, oe);
      ...
      //     person_label_id_, root, person_label_id_, knows_label_id_);
      std::vector<gs::MutableNbr<Date>> ie;
      txn.GetIncomingGraphView<Date>(person_label_id_, person_label_id_, knows_label_id_).read(root, ie);
      ...
      for (auto &v : vec)
      {
        ...
        person_firstName_col_.read(v.v, item_v);
        output.put_string_view({item_v.data(), item_v.size()});
        person_lastName_col_.read(v.v, item_v);
        output.put_string_view({item_v.data(), item_v.size()});
      }
      return true;
    }
