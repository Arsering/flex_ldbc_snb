...
namespace gs
{
  ...
  public:
    ...
    bool Query(Decoder &input, Encoder &output) override
    {
      ...
      {
        post_creationDate_col_.read(lid, items);
        output.put_long(reinterpret_cast<gs::Date *>(items.data())[0].milli_second);
        ...
        post_length_col_.read(lid, items);
        reinterpret_cast<int *>(items.data())[0] == 0 ? post_imageFile_col_.read(lid, items) : post_content_col_.read(lid, items);
        // output.put_string_view(content);
        output.put_string_view({items.data(), items.size()});
        return true;
      }
      else if (txn.GetVertexIndex(comment_label_id_, id, lid))
      {
        comment_creationDate_col_.read(lid, items);
        output.put_long(reinterpret_cast<gs::Date *>(items.data())[0].milli_second);

        // output.put_long(comment_creationDate_col_.get_view(lid).milli_second);

        // const auto &content = comment_content_col_.get_view(lid);
        comment_content_col_.read(lid, items);
        // output.put_string_view(content);
        output.put_string_view({items.data(), items.size()});
        return true;
      }
