  ...
  public:
    ...
    bool Query(Decoder &input, Encoder &output) override
    {
      ...
      if (txn.GetVertexIndex(post_label_id_, id, lid))
      {
        ...
        post_hasCreator_person_out.read(lid, items);
      ...
      else if (txn.GetVertexIndex(comment_label_id_, id, lid))
      {
        ...
        std::vector<gs::MutableNbr<grape::EmptyType>> items;
        comment_hasCreator_person_out.read(lid, items);
        assert(items[0].timestamp.load() <= txn.GetTimeStamp());
      ...
      person_firstName_col_.read(v, item_v);
      output.put_string_view({item_v.data(), item_v.size()});
      person_lastName_col_.read(v, item_v);
      output.put_string_view({item_v.data(), item_v.size()});
      return true;
    }
