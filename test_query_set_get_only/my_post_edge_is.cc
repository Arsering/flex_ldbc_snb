...
class POST_EDGE_IS : public AppBase {
public:
  POST_EDGE_IS(GraphDBSession &graph)
  ...
  bool Query(Decoder &input, Encoder &output) override {
    ...
    // auto person_place =
    // gbp::BufferBlock::Ref<MutableNbr<grape::EmptyType>>(item).neighbor; item
    // = place_name_col_.get(person_place); output.put_buffer_object(item);
    ...
    // if (comment_replyOf_post_in.) {
    // statements
    // }
    // auto ie = comment_replyOf_post_in.get_edges(lid);
...
extern "C"
{
  ...
  {
    gs::POST_EDGE_IS *app = new gs::POST_EDGE_IS(db);
    return static_cast<void *>(app);
  }

  void DeleteApp(void *app)
  {
    gs::POST_EDGE_IS *casted = static_cast<gs::POST_EDGE_IS *>(app);
    delete casted;
  }
}
