  ...
  public:
    ...
    bool Query(Decoder &input, Encoder &output) override
    {
      ...
      for (; post_ie.is_valid(); post_ie.next())
      {
        ...
        for (; ie.is_valid(); ie.next())
        {
          ...
          if (pq.size() < 20)
          {
            ...
            pq.emplace(u, gbp::BufferBlock::RefSingle<Date>(item).milli_second,
                       txn.GetVertexId(comment_label_id_, u));
          }
          else
          {
            ...
            auto item = comment_creationDate_col_.get(u);
            int64_t creationDate =
                gbp::BufferBlock::RefSingle<Date>(item).milli_second;
#endif
      ...
      for (auto i = vec.size(); i > 0; i--)
      {
        ...
        auto p = gbp::BufferBlock::RefSingle<gs::MutableNbr<grape::EmptyType>>(item).neighbor;
        output.put_long(txn.GetVertexId(person_label_id_, p));
